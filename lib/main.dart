...
class RegistryController extends ChangeNotifier {
  RegistryController({required this.repository});
  ...
  Future<void> loadData() async {
    ...
    try {
      ...
      try {
        ...
        // Handle permission errors gracefully on first load
        _bookings = [];
        _expenses = [];
      }
      ...
      _bookingsSub = bookingsStream.listen(
        ...
        onError: (Object e) {
          // Only show error if it's not a permission-denied from empty collections
      ...
      // Silently handle initial load errors for permission denied
      if (!e.toString().contains('permission-denied')) {
...
class _RegistryHomePageState extends State<RegistryHomePage> {
  late final RegistryController _controller;
  int _index = 0;
  ...
  // Web client ID is required for native Android Google Sign-In token exchange.
...
Future<void> _signOutToAnonymous() async {
    try {
      // 1. First, cleanly sign out from Google
      if (!kIsWeb) {
        await _googleSignIn.signOut();
      }
      
      // 2. Clear out the Firebase instance and IMMEDIATELY sign in anonymously
      await FirebaseAuth.instance.signOut();
      await FirebaseAuth.instance.signInAnonymously(); // 👈 Guarantees auth is never null during a rebuild
  ...
  void initState() {
    super.initState();
    _controller = RegistryController(repository: FirestoreRegistryRepository());
    ...
    return AnimatedBuilder(
      ...
      builder: (context, _) {
        final pages = [
          ...
          ExpensesScreen(controller: _controller),
          ReportsScreen(controller: _controller),
        ];
...
class DashboardScreen extends StatelessWidget {
  ...
  final RegistryController controller;
  ...
  Future<void> _deleteBooking(Booking booking) async {
    final shouldDelete = await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Text('Delete booking for ${booking.name}?'),
        actions: [
          TextButton(
...
class _ReportsScreenState extends State<ReportsScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    final monthBookings = widget.controller._bookings
        .where((b) => b.checkIn.year == _selectedYear && b.checkIn.month == _selectedMonth)
        .toList();

    // Filter expenses for selected month
    final monthExpenses = widget.controller._expenses
        .where((e) => e.date.year == _selectedYear && e.date.month == _selectedMonth)
        .toList();
